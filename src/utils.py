...
def get_coarse_grained_samples(classes, fls_im, fls_sk, set_type='train', filter_sketch=True):
    idx_imp_ret = np.array([], dtype=np.int)
    idx_sk_ret = np.array([], dtype=np.int)
    idx_imn_ret = np.array([], dtype=np.int)
    idx_teim_ret = np.array([], dtype=np.int)
    idx_tesk_ret = np.array([], dtype=np.int)
    ...
    for i, c in enumerate(classes):
        clss_ne = np.setdiff1d(classes, c)    #负样本可能的集合
        c_n = random.choice(clss_ne)         #选取负样本的类

        idx1 = np.where(clss_im == c)[0]
        idx2 = np.where(clss_sk == c)[0]
        idx3 = np.where(clss_im == c_n)[0]
        idx3 = np.repeat(idx3[0], len(idx1))

        idx_imp_ret = np.concatenate((idx_imp_ret, idx1[:int(0.4*len(idx1))]), axis=0)
        idx_sk_ret = np.concatenate((idx_sk_ret, idx2[:int(0.4*len(idx1))]), axis=0)
        idx_imn_ret = np.concatenate((idx_imn_ret, idx3[:int(0.4*len(idx1))]), axis=0)
        idx_teim_ret = np.concatenate((idx_teim_ret, idx1[-int(0.3*len(idx1)):]), axis=0)
        idx_tesk_ret = np.concatenate((idx_tesk_ret, idx2[-int(0.3*len(idx1)):]), axis=0)

    return idx_imp_ret, idx_sk_ret, idx_imn_ret, idx_teim_ret, idx_tesk_ret


def load_files_sketchy_zeroshot(root_path, filter_sketch=False, photo_dir='photo',
    ...
    idx_tr_imp, idx_tr_sk, idx_tr_imn, _, _ = get_coarse_grained_samples(classes, fls_im, fls_sk, set_type='train',
                                                                   filter_sketch=filter_sketch)
    _, _, _, idx_te_imp, idx_te_sk = get_coarse_grained_samples(classes, fls_im, fls_sk, set_type='test',
                                                                   filter_sketch=filter_sketch)
    ...
    splits['tr_fls_imp'] = fls_im[idx_tr_imp]
    splits['te_fls_imp'] = fls_im[idx_te_imp]

    splits['tr_clss_imp'] = clss_im[idx_tr_imp]
    splits['te_clss_imp'] = clss_im[idx_te_imp]

    splits['tr_fls_imn'] = fls_im[idx_tr_imn]
    #splits['te_fls_imn'] = fls_im[idx_te_imn]

    splits['tr_clss_imn'] = clss_im[idx_tr_imn]
    #splits['te_clss_imn'] = clss_im[idx_te_imn]
    #print(len(splits['tr_fls_sk']))
...
def load_files_tuberlin_zeroshot(root_path, photo_dir='images', sketch_dir='sketches', photo_sd='', sketch_sd=''):
    ...
    fls_im = glob.glob(os.path.join(path_im, '*', '*.JPEG'))
